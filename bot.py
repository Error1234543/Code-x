...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # NOTE: 'name' parameter removed — it was causing %20 space encoding in the URL
        ...
        raw_link = invite.invite_link.strip()  # strip any accidental whitespace
        logger.info(f"Generated invite link for user {user.id}: {raw_link}")

        # Validate link looks correct before sending
        if not raw_link.startswith("https://t.me/+") and not raw_link.startswith("https://t.me/joinchat/"):
            logger.error(f"Unexpected invite link format: {raw_link}")
            raise ValueError(f"Invite link format invalid: {raw_link}")

        # Shorten via VPLink
        short_url      = await vplink.shorten(raw_link)
        access_expires = datetime.now() + timedelta(hours=ACCESS_HOURS)
        db.create_access(user.id, channel_id, channel_name, raw_link, access_expires)
        ...
        await query.edit_message_text(
            f"❌ *Kuch gadbad ho gayi!*\n\nError: `{str(e)[:200]}`\n\nThodi der baad try karo ya admin se contact karo.",
            parse_mode="Markdown"
        )
